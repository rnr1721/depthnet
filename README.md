...
- **Unified Memory (Knowledge)**: An optional single interface over all five memory subsystems. The agent states the *nature* of what it knows — episode, person, relation, skill, or note — and a deterministic router (no LLM in the loop) writes it to exactly one home, while recall fans out across every subsystem and merges the results. Distinct from RAG: RAG is a passive, pre-assembled retrieval layer; knowledge is active, driven by the agent on demand, and bypasses RAG entirely on read. Two input modes — the model emits the routing intent directly, or a lightweight formulator preset translates plain natural-language intent into it. Works for instrumental agents (a reliable remember/recall tool) and subjective ones (a single memory surface) alike — the difference is prompt and RAG config, not the mechanism. [→](docs/plugins/knowledge.md)
...
| **Knowledge** (`knowledge`) | **Facade over the five memory plugins below** (journal, vectormemory, ontology, person, skill) — one tool for remember / recall / relate / forget; a deterministic router picks the home. Enable to replace the five separate tools with one interface. | [→](docs/plugins/knowledge.md) |
...
DepthNet's memory is five specialised plugins — **journal** (episodes), **vectormemory** (semantic notes), **ontology** (a world-model graph), **person** (people), and **skill** (reusable know-how). Each works on its own and can be enabled independently, exactly as before.

The optional **`knowledge`** plugin is a thin, deterministic *facade* over those five — not a sixth store. Instead of enabling all five and carrying five instruction blocks in the prompt, the agent uses one tool and expresses the **nature** of a piece of knowledge; a deterministic router maps each nature to its single home on write, and fans out across every subsystem on recall. No model call decides the routing — the mapping is fixed code.

- **Write is unambiguous** — one nature, one home. No duplicate copies scattered across subsystems.
- **Recall is a fan-out** — every source is searched in parallel and merged into one lean result, independently of RAG (which may be disabled). RAG stays a passive retrieval layer; knowledge is active and agent-driven.
- **Two input modes** — *direct* (the model produces the routing intent itself, no extra model call) and *formulator* (the model speaks plainly; a small preset translates the intent). Both converge on the same deterministic router.
