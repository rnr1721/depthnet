                            ...
                            <div :class="[
                                ...
                                <div class="flex items-center justify-between">
                                    <h3 id="modal-title" :class="[
                                        ...
                                        {{ t('mm_edit_memory_item') }} #{{ item?.position }}
                                    </h3>
                                    <button @click="close" :class="[
                                        'rounded-lg p-2 transition-colors hover:bg-opacity-75 focus:outline-none focus:ring-2 focus:ring-yellow-500',
                            ...
                            <form @submit.prevent="submit">
                                <div class="px-6 py-4">
                                        ...
                                        <textarea v-model="form.content" rows="6"
                                            :placeholder="t('mm_enter_memory_content')" :class="[
                                                ...
                                                form.errors.content
                                                    ...
                                                    : 'ring-gray-300 focus:ring-yellow-500',
                                                isDark
                                                    ? (form.errors.content
                                    ...
                                    <div v-if="item" :class="[
                                        ...
                                        <div><strong>{{ t('mm_preset') }}:</strong> {{ preset?.name }}</div>
                                        <div><strong>{{ t('mm_position') }}:</strong> #{{ item.position }}</div>
                                        <div><strong>{{ t('mm_original_length') }}:</strong> {{ item.content?.length || 0
                                            }} {{ t('mm_chars') }}</div>
                                    </div>
                                </div>
                                ...
                                <div :class="[
                                    ...
                                    <button type="submit" :disabled="form.processing || !form.content?.trim()" :class="[
                                        'w-full sm:w-auto px-6 py-2 rounded-xl font-medium transition-all focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2',
                                        'disabled:opacity-50 disabled:cursor-not-allowed',
                                        isDark
                                            ? 'bg-yellow-600 hover:bg-yellow-700 text-white focus:ring-offset-gray-800'
                                        ...
                                        <span v-else>{{ t('mm_update_item') }}</span>
                                    </button>
                                </div>
                            </form>
                        </div>
...
const submit = () => {
    ...
    form.put(route('admin.memory.update', props.item?.id), {
        onSuccess: () => {
            close();
            emit('success');
        }
