...
class GeminiModel implements AIModelEngineInterface
{
     ...
     *
     * Gemini via Google AI Studio uses an OpenAI-compatible endpoint,
     * so tool_calls work identically to other OpenAI-compatible providers.
     *
    ...
    public function generate(AiModelRequestInterface $request): AiModelResponseInterface
    {
        try {
            ...
            if ($finishReason === 'tool_calls' && !empty($message['tool_calls'])) {
                $this->logger->info('Gemini tool_calls received', [
                    'model' => $this->model,
            ...
            if (!isset($message['content'])) {
                ...
                throw new AiModelException(
                    config('ai.global.error_messages.invalid_format', 'Invalid response format from Google Gemini API')
                );
            }
    ...
    /**
     * Build messages array for the Google Gemini API (OpenAI-compatible endpoint).
     *
     * Role mapping:
     *   user    → role: user
     ...
     *
     * @param  AiModelRequestInterface $request
     * @return array                   Messages array ready for the Gemini API
     */
