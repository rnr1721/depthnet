                        ...
                        <div v-if="show"
                                ...
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <h3 :class="['text-lg font-semibold', isDark ? 'text-white' : 'text-gray-900']">
                                            {{ t('wm_clear_confirm_btn') }}
                                        </h3>
                                    </div>
                                    <button @click="close"
                                        :class="['rounded-lg p-2 transition-colors focus:outline-none focus:ring-2 focus:ring-gray-500', isDark ? 'hover:bg-gray-700 text-gray-400' : 'hover:bg-gray-100 text-gray-600']">
                            ...
                            <div class="px-6 py-5">
                                <p :class="['text-sm leading-relaxed', isDark ? 'text-gray-300' : 'text-gray-600']">
                                    {{ t('wm_clear_confirm') }} {{ t('wm_preset') }}: {{ preset?.name }}
                                </p>
                                <p :class="['mt-3 text-sm font-medium', isDark ? 'text-red-400' : 'text-red-600']">
                                    {{ t('wm_irreversible') }}
                                </p>
                            </div>
                            ...
                            <div
                                ...
                                <button @click="confirm" :disabled="processing"
                                    ...
                                    <span v-else>{{ t('wm_clear_title') }}</span>
                                </button>
                            </div>
                        </div>
                    </Transition>
...
const confirm = () => {
    ...
    router.post(route('admin.workspace.clear'), {
