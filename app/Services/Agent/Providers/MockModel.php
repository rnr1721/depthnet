...
class MockModel implements AIModelEngineInterface
{
    ...
    public function __construct(
        ...
        $this->config = array_merge($this->getDefaultConfig(), $defaultConfig);

        // Set server URL from config if not provided
        $this->serverUrl = $this->config['server_url'] ?? 'http://localhost:8080';
    ...
    public function generate(
        ...
        $currentDophamineLevel = $request->getDopamineLevel();
