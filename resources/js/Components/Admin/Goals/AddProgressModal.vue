                ...
                <div class="flex min-h-screen items-end justify-center px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                    ...
                    <Transition enter-active-class="transition ease-out duration-300"
                        ...
                        <div v-if="show"
                            ...
                            <div :class="['px-6 py-4 border-b', isDark ? 'border-gray-700' : 'border-gray-200']">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <div>
                                            <h3 :class="['text-lg font-semibold', isDark ? 'text-white' : 'text-gray-900']">{{ t('gm_add_progress') }}</h3>
                            ...
                            <div class="px-6 py-4">
                                <label :class="['block text-sm font-medium mb-2', isDark ? 'text-gray-300' : 'text-gray-700']">{{ t('gm_progress_note') }} *</label>
                                <textarea v-model="form.content" rows="5" :placeholder="t('gm_progress_placeholder')"
                                    :class="['w-full rounded-xl border-0 ring-1 ring-inset focus:ring-2 focus:ring-amber-500 transition-all resize-none px-4 py-3 text-sm', form.errors.content ? 'ring-red-500' : '', isDark ? 'bg-gray-700 text-white placeholder-gray-400 ring-gray-600' : 'bg-gray-50 text-gray-900 placeholder-gray-500 ring-gray-300']"></textarea>
                                <div v-if="form.errors.content" class="mt-1 text-xs text-red-500">{{ form.errors.content }}</div>
                                <div :class="['text-xs mt-1 text-right', isDark ? 'text-gray-500' : 'text-gray-400']">{{ form.content?.length || 0 }} {{ t('gm_chars') }}</div>
                            </div>
                            ...
                            <div :class="['px-6 py-4 border-t flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-3 space-y-3 space-y-reverse sm:space-y-0', isDark ? 'border-gray-700' : 'border-gray-200']">
                                ...
                                <button @click="submit" :disabled="form.processing || !form.content?.trim()"
                                    ...
                                    <span v-else>{{ t('gm_save_progress') }}</span>
                                </button>
                            </div>
                        </div>
                    </Transition>
...
const form = useForm({ preset_id: null, goal_number: null, content: '' });
const handleEscape = (e) => { if (e.key === 'Escape' && show.value) close(); };
const close = () => { show.value = false; form.reset(); form.clearErrors(); document.body.style.overflow = ''; };
const submit = () => {
    form.preset_id   = props.preset?.id;
    form.goal_number = props.goal?.number;
    form.post(route('admin.goals.progress'), { onSuccess: () => { close(); emit('success'); } });
};
