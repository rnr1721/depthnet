...
class OpenAIModel implements AIModelEngineInterface
{
     ...
     *
     * OpenAI is the canonical tool_calls provider — the format used by all
    ...
    public function generate(AiModelRequestInterface $request): AiModelResponseInterface
    {
        try {
            ...
            if ($finishReason === 'tool_calls' && !empty($message['tool_calls'])) {
                $this->logger->info('OpenAI tool_calls received', [
                    'model' => $this->model,
            ...
            if (!isset($message['content'])) {
                ...
                throw new AiModelException(
                    config('ai.global.error_messages.invalid_format', 'Invalid response format from OpenAI API')
                );
            }
     ...
     *
     * @param  AiModelRequestInterface $request
     * @return array                   Messages array ready for the OpenAI API
     */
