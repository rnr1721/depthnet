                    ...
                    <Transition enter-active-class="transition ease-out duration-300"
                        ...
                        <div v-if="show"
                            ...
                            <div :class="['px-6 py-4 border-b', isDark ? 'border-gray-700' : 'border-gray-200']">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <h3 :class="['text-lg font-semibold', isDark ? 'text-white' : 'text-gray-900']">{{ t('pm_add_fact') }}</h3>
                                    </div>
                                    <button @click="close" :class="['rounded-lg p-2 transition-colors focus:outline-none focus:ring-2 focus:ring-rose-500', isDark ? 'hover:bg-gray-700 text-gray-400' : 'hover:bg-gray-100 text-gray-600']">
                            ...
                            <div class="px-6 py-4 space-y-4">
                                <!-- Person name -->
                                <div>
                                    <label :class="['block text-sm font-medium mb-2', isDark ? 'text-gray-300' : 'text-gray-700']">{{ t('pm_person_name') }} *</label>
                                    <input v-model="form.person_name" type="text" :placeholder="t('pm_person_name_placeholder')"
                                    ...
                                    <div v-if="form.errors.person_name" class="mt-1 text-xs text-red-500">{{ form.errors.person_name }}</div>
                                </div>

                                <!-- Fact content -->
                                <div>
                                    <label :class="['block text-sm font-medium mb-2', isDark ? 'text-gray-300' : 'text-gray-700']">{{ t('pm_fact_content') }} *</label>
                                    ...
                                    <div :class="['text-xs mt-1 text-right', isDark ? 'text-gray-500' : 'text-gray-400']">{{ form.content?.length || 0 }} {{ t('pm_chars') }}</div>
                                </div>
                            ...
                            <div :class="['px-6 py-4 border-t flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-3 space-y-3 space-y-reverse sm:space-y-0', isDark ? 'border-gray-700' : 'border-gray-200']">
                                ...
                                <button @click="submit" :disabled="form.processing || !form.person_name?.trim() || !form.content?.trim()"
...
const props = defineProps({
    modelValue: Boolean,
    preset: Object,
    initialPersonName: { type: String, default: '' },
    isDark: Boolean,
});
...
watch(() => show.value, (visible) => {
    if (visible) {
        form.person_name = props.initialPersonName || '';
...
watch(() => props.initialPersonName, (val) => {
    if (show.value) form.person_name = val || '';
});
