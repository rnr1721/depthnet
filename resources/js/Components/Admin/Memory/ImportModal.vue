                            ...
                            <form @submit.prevent="submit" enctype="multipart/form-data">
                                <div class="px-6 py-4 space-y-6">
                                    ...
                                    <div v-if="importMethod === 'file'">
                                        ...
                                        <div :class="[
                                            ...
                                            <input ref="fileInput" type="file" accept=".txt" @change="handleFileSelect"
                                                class="sr-only" />
                                    ...
                                    <div v-else>
                                        ...
                                        <textarea v-model="textContent" rows="8"
                                            :placeholder="t('mm_paste_memory_content_here')" :class="[
                                        ...
                                        <div class="flex justify-between items-center mt-2">
                                            ...
                                            <div class="text-xs opacity-50 ml-auto">
                                                {{ textContent?.length || 0 }} {{ t('mm_chars') }}
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div :class="[
                                        ...
                                        <label :class="[
                                            ...
                                            <div>
                                                <div class="text-sm font-medium">{{ t('mm_replace_existing_memory') }}
                                                </div>
                                                <div :class="['text-xs', isDark ? 'text-gray-400' : 'text-gray-600']">
                                                    {{ t('mm_clear_current_memory_before_import') }}
                                                </div>
                                            </div>
                                        </label>
                                    </div>
                                ...
                                <div :class="[
                                    ...
                                    <button type="submit" :disabled="form.processing || !canSubmit" :class="[
                                        ...
                                        <span v-else>{{ t('mm_import_memory') }}</span>
                                    </button>
                                </div>
                            </form>
                        </div>
...
const handleFileSelect = (event) => {
    ...
    if (file) {
        if (file.type !== 'text/plain' && !file.name.endsWith('.txt')) {
            alert(t('mm_please_select_txt_file'));
            return;
        }
        if (file.size > 1024 * 1024) { // 1MB
            alert(t('mm_file_too_large'));
            return;
        }
...
const submit = () => {
    ...
    form.post(route('admin.memory.import'), {
        onSuccess: () => {
            close();
            emit('success');
        }
