...
/**
 * Job for stopping sandbox containers with progress tracking
 *
 * This job handles graceful shutdown of sandbox containers with configurable timeout
 */
class StopSandboxJob implements ShouldQueue
{
    ...
    public function handle(SandboxOperationServiceInterface $operationService): void
    {
        ...
        try {
            $sandbox = $operationService->stopSandbox(
                $this->operationId,
                $this->sandboxId,
                $this->timeout
            );

            Log::info('StopSandboxJob completed successfully', [
            ...
            Log::error('StopSandboxJob failed', [
                'operation_id' => $this->operationId,
                'sandbox_id' => $this->sandboxId,
                'error' => $e->getMessage(),
                'user_id' => $this->userId
    ...
    {
        Log::error('StopSandboxJob failed with exception', [
            'operation_id' => $this->operationId,
            'sandbox_id' => $this->sandboxId,
