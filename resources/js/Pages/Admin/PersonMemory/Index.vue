    ...
    <div :class="['min-h-screen transition-colors duration-300', isDark ? 'bg-gray-900' : 'bg-gray-50']">
        ...
        <main class="relative">
            <div class="absolute inset-0 overflow-hidden pointer-events-none">
                <div :class="['absolute -top-40 -right-40 w-80 h-80 rounded-full opacity-10 blur-3xl', isDark ? 'bg-rose-500' : 'bg-rose-300']"></div>
                <div :class="['absolute -bottom-40 -left-40 w-80 h-80 rounded-full opacity-10 blur-3xl', isDark ? 'bg-pink-500' : 'bg-pink-300']"></div>
            </div>
            ...
            <div class="relative max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
                ...
                <div :class="['mb-8 backdrop-blur-sm border shadow-xl rounded-2xl overflow-hidden transition-all p-6', isDark ? 'bg-gray-800 bg-opacity-90 border-gray-700' : 'bg-white bg-opacity-90 border-gray-200']">
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
                        <div class="flex-1">
                            <label :class="['block text-sm font-medium mb-2', isDark ? 'text-gray-300' : 'text-gray-700']">{{ t('pm_select_preset') }}</label>
                        ...
                        <div class="flex gap-6">
                            <div class="text-center">
                                <div :class="['text-2xl font-bold', isDark ? 'text-white' : 'text-gray-900']">{{ people.length }}</div>
                                <div :class="['text-xs', isDark ? 'text-gray-400' : 'text-gray-600']">{{ t('pm_stats_people') }}</div>
                            </div>
                            <div class="text-center">
                                <div class="text-2xl font-bold text-rose-500">{{ totalFacts }}</div>
                                <div :class="['text-xs', isDark ? 'text-gray-400' : 'text-gray-600']">{{ t('pm_stats_facts') }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div :class="['mb-6 backdrop-blur-sm border shadow-xl rounded-2xl overflow-hidden transition-all p-4', isDark ? 'bg-gray-800 bg-opacity-90 border-gray-700' : 'bg-white bg-opacity-90 border-gray-200']">
                    <div class="flex items-center justify-between">
                        <p :class="['text-sm', isDark ? 'text-gray-400' : 'text-gray-500']">{{ t('pm_description') }}</p>
                        ...
                        <div v-for="person in people" :key="person.name">
                            ...
                            <div :class="['flex items-center justify-between px-5 py-4 cursor-pointer select-none transition-colors', isDark ? 'hover:bg-gray-750' : 'hover:bg-gray-50']"
                                ...
                                <div class="flex items-center space-x-3">
                                    <!-- Avatar initial -->
                                    <div class="w-9 h-9 rounded-full bg-gradient-to-br from-rose-400 to-pink-600 flex items-center justify-center flex-shrink-0">
                                        <span class="text-white text-sm font-bold">{{ person.name.charAt(0).toUpperCase() }}</span>
                                    </div>
                                    <div>
                                        <span :class="['font-medium', isDark ? 'text-white' : 'text-gray-900']">{{ person.name }}</span>
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <button @click.stop="openAddFactForPerson(person.name)"
                                    ...
                                    <!-- Forget person -->
                                    <button @click.stop="forgetPerson(person.name)"
                                ...
                                <div v-if="expandedPeople.has(person.name)"
                                    ...
                                    <div v-for="fact in person.facts" :key="fact.id"
                                        ...
                                        <div class="flex items-start space-x-3 flex-1 min-w-0">
                                            <span :class="['font-mono text-xs font-bold mt-0.5 flex-shrink-0 w-6 text-right', isDark ? 'text-rose-400' : 'text-rose-600']">
                                                {{ fact.number }}.
                                            </span>
                                            <p :class="['text-sm leading-relaxed', isDark ? 'text-gray-300' : 'text-gray-700']">{{ fact.content }}</p>
                                        </div>
                                        <button @click="deleteFact(person.name, fact.number)"
                ...
                <div v-else
                    ...
                    <div class="w-16 h-16 mx-auto mb-4 bg-gradient-to-br from-rose-400 to-pink-600 rounded-full flex items-center justify-center">
                        <svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"></path>
                        </svg>
                    </div>
                    <h3 :class="['text-lg font-medium mb-2', isDark ? 'text-white' : 'text-gray-900']">{{ t('pm_no_people') }}</h3>
                    <p :class="['text-sm mb-6', isDark ? 'text-gray-400' : 'text-gray-600']">{{ t('pm_no_people_description') }}</p>
                    <button @click="showAddModal = true"
        ...
        <AddPersonFactModal
            ...
            :initial-person-name="addModalPersonName"
            :isDark="isDark"
            @success="refreshData"
        />
        <ClearPersonMemoryModal
...
const openAddFactForPerson = (name) => {
    addModalPersonName.value = name;
...
const deleteFact = (personName, factNumber) => {
    ...
    router.delete(route('admin.person-memory.delete-fact'), {
        ...
        onSuccess: () => {
            // Keep person expanded after fact deletion
...
const forgetPerson = (personName) => {
    if (!confirm(t('pm_confirm_forget', { name: personName }))) return;
    router.delete(route('admin.person-memory.forget', personName), {
...
onMounted(() => {
    ...
    // Expand first person by default if only one exists
