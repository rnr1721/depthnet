                        ...
                        <div v-if="show"
                            ...
                            <div class="px-6 py-4 space-y-4">
                                ...
                                <div>
                                    ...
                                    <input v-model="form.key" type="text" placeholder="e.g. plan, draft, notes"
                                        :class="['w-full rounded-xl border-0 ring-1 ring-inset focus:ring-2 transition-all px-4 py-3 font-mono text-sm', keyError ? 'ring-red-500 focus:ring-red-500' : 'focus:ring-violet-500', isDark ? 'bg-gray-700 text-white placeholder-gray-400 ring-gray-600' : 'bg-gray-50 text-gray-900 placeholder-gray-500 ring-gray-300']"
                                        @input="keyError = ''" />
                                    <div v-if="keyError" class="mt-1 text-xs text-red-500">{{ keyError }}</div>
                                    <div v-else-if="form.errors.key" class="mt-1 text-xs text-red-500">{{
                                        form.errors.key }}</div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea v-model="form.value" rows="6"
                                        placeholder="Enter the content for this workspace key..."
                                        :class="['w-full rounded-xl border-0 ring-1 ring-inset focus:ring-2 focus:ring-violet-500 transition-all resize-none px-4 py-3 text-sm', form.errors.value ? 'ring-red-500' : '', isDark ? 'bg-gray-700 text-white placeholder-gray-400 ring-gray-600' : 'bg-gray-50 text-gray-900 placeholder-gray-500 ring-gray-300']"></textarea>
                                    <div v-if="form.errors.value" class="mt-1 text-xs text-red-500">{{ form.errors.value
                                        }}</div>
                                    <div
                                        :class="['text-xs mt-1 text-right', isDark ? 'text-gray-500' : 'text-gray-400']">
                                        {{ form.value?.length || 0 }} {{ t('wm_chars') }}
                                    </div>
                                </div>
                                ...
                                <div v-if="preset"
                                    ...
                                    <strong>{{ t('wm_preset') }}</strong> {{ preset.name }}
                                </div>
                            </div>
                            ...
                            <div
                                ...
                                <button @click="submit"
                                    :disabled="form.processing || !form.key?.trim() || !form.value?.trim()"
                                    ...
                                    <span v-else>{{ t('wm_add_entry') }}</span>
                                </button>
                            </div>
                        </div>
                    </Transition>
...
const form = useForm({ preset_id: null, key: '', value: '' });
const keyError = ref('');
...
const submit = () => {
    ...
    if (props.existingKeys.includes(form.key.trim())) {
        keyError.value = `Key "${form.key.trim()}" already exists. It will be overwritten.`;
        // allow submit anyway — set() is upsert
    }

    form.preset_id = props.preset?.id;
    form.post(route('admin.workspace.store'), {
