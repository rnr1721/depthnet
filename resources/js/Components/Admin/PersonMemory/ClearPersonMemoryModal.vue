                        ...
                        <div v-if="show"
                            ...
                            <div :class="['px-6 py-4 border-b', isDark ? 'border-gray-700' : 'border-gray-200']">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center space-x-3">
                                        ...
                                        <h3 :class="['text-lg font-semibold', isDark ? 'text-white' : 'text-gray-900']">{{ t('pm_clear_all') }}</h3>
                                    </div>
                            ...
                            <div class="px-6 py-5">
                                <p :class="['text-sm leading-relaxed', isDark ? 'text-gray-300' : 'text-gray-600']">
                                    {{ t('pm_clear_confirm', { preset: preset?.name }) }}
                                </p>
                                <p :class="['mt-3 text-sm font-medium', isDark ? 'text-red-400' : 'text-red-600']">{{ t('pm_irreversible') }}</p>
                            </div>
                            ...
                            <div :class="['px-6 py-4 border-t flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-3 space-y-3 space-y-reverse sm:space-y-0', isDark ? 'border-gray-700' : 'border-gray-200']">
                                ...
                                <button @click="confirm" :disabled="processing"
                                    ...
                                    <span v-else>{{ t('pm_clear_confirm_btn') }}</span>
                                </button>
                            </div>
                        </div>
                    </Transition>
...
const confirm = () => {
    ...
    router.post(route('admin.person-memory.clear'), { preset_id: props.preset?.id }, {
