                                ...
                                <div>
                                    <label
                                        :class="['block text-sm font-medium mb-2', isDark ? 'text-gray-300' : 'text-gray-700']">{{
                                            t('at_task_title') }} *</label>
                                    <input v-model="form.title" type="text"
                                        :placeholder="t('at_task_title_placeholder')"
                                        :class="['w-full rounded-xl border-0 ring-1 ring-inset focus:ring-2 transition-all px-4 py-3', form.errors.title ? 'ring-red-500 focus:ring-red-500' : 'focus:ring-indigo-500', isDark ? 'bg-gray-700 text-white placeholder-gray-400 ring-gray-600' : 'bg-gray-50 text-gray-900 placeholder-gray-500 ring-gray-300']" />
                                    <div v-if="form.errors.title" class="mt-1 text-xs text-red-500">{{ form.errors.title
                                        }}</div>
                                </div>
                                <div>
                                    <label
                                        :class="['block text-sm font-medium mb-2', isDark ? 'text-gray-300' : 'text-gray-700']">
                                        {{ t('at_task_description') }}
                                        <span
                                            :class="['ml-1 text-xs font-normal', isDark ? 'text-gray-500' : 'text-gray-400']">{{
                                                t('at_optional') }}</span>
                                    </label>
                                    <textarea v-model="form.description" rows="3"
                                        :placeholder="t('at_task_description_placeholder')"
                                        :class="['w-full rounded-xl border-0 ring-1 ring-inset focus:ring-2 focus:ring-indigo-500 transition-all px-4 py-3 resize-none', isDark ? 'bg-gray-700 text-white placeholder-gray-400 ring-gray-600' : 'bg-gray-50 text-gray-900 placeholder-gray-500 ring-gray-300']"></textarea>
                                </div>
                                <div>
                                    <label
                                        :class="['block text-sm font-medium mb-2', isDark ? 'text-gray-300' : 'text-gray-700']">
                                    ...
                                    <select v-model="form.assigned_role"
                                        ...
                                        <option value="">{{ t('at_unassigned') }}</option>
                                        <option v-for="role in agent?.roles" :key="role.id" :value="role.code">
                                            {{ role.code }}
                                        </option>
                                    </select>
                                    <p :class="['mt-1 text-xs', isDark ? 'text-gray-500' : 'text-gray-400']">{{
                                        t('at_assigned_role_hint') }}</p>
                                </div>
                            </div>
...
const submit = () => {
    form.agent_id = props.agent?.id;
    form.post(route('admin.agent-tasks.store'), {
